...
# docker buildx build --tag cosmolotl --file .pi/Dockerfile --platform linux/arm64 --cache-from cosmolotl:cache --cache-to cosmolotl:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar cosmolotl
...
sudo bash -c "echo cosmolotl > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/cosmolotl.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/cosmolotl.img
